...
const ChatList = () => {
  ...
  const {
    ...
    error,
    refetch,
  } = useGetChatsQuery({ pollingInterval: 10000 });
      ...
      <div className="chat-list">
        {chats?.map((chat) => (
          <div
            ...
            onClick={() =>
              handleChangeChatAndUser(chat.id, chat.interlocutor?.id)
            }
          >
            ...
            <div className="chat-content">
              <div className="chat-header">
                <span className="chat-username">
                  {chat.interlocutor?.username || 'Unknown'}
                </span>
              </div>
              <div className="chat-preview">
                <p className="chat-last-message">
                  {chat.lastMessage?.content || '📎file'}
                </p>
                {!chat.lastMessage?.is_read &&
                chat.lastMessage?.sender_id === chat.interlocutor.id ? (
