...
class ChatService {
  ...
  async getUserChats(userId) {
    ...
    return Promise.all(
      chats.map((chat) => this.buildConversationPreview(chat.id, userId)),
    );
  }
  ...
  async findUserChat(fromUserId, toUserId) {
    ...
    const privateChat = chats.find((chat) => {
      ...
      return (
        members.includes(fromUserId) &&
        members.includes(toUserId) &&
        members.length === 2
      );
    });
  ...
  async buildConversationPreview(chatId, currentUserId, transaction = null) {
      ...
      include: [
        {
          model: UserModel,
          as: "members",
          attributes: ["id", "username", "last_seen", ["avatar_url", "avatar"]],
        },
        {
          model: MessageModel,
          as: "messages",
    ...
    return {
      ...
      title: chat.is_group ? chat.group_name : otherMember?.username,

      avatar: chat.is_group ? chat.group_avatar : otherMemberData?.avatar,

      isGroup: chat.is_group,

      unreadCount,

      lastMessage: getPreviewText(lastMessage),

      updatedAt: lastMessage?.sent_at ?? chat.created_at,
